...
**Quem roda `bun run art` (ou qualquer variante futura) é sempre o Rodrigo, nunca o Codex por conta
própria** — geração de imagem consome GPU local por vários segundos a minutos por variante, e rodar sem avisar
pode travar a máquina no meio de outra coisa. Codex pode editar `portrait.json`/prompts/pipeline, rodar
...
dedicada pra preencher/ajustar `geracaoArt` de uma espécie via entrevista: `.Codex/skills/gerar-art-portrait/`
(`/gerar-art-portrait`).
...
**Nenhum commit deste repositório leva trailer `Co-Authored-By:` de Codex/Anthropic** — não escreva essa linha
